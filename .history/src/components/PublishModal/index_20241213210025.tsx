  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (result.code === 200 && result.data) {
        ...
        const currentTime = `${beijingHours.toString().padStart(2, '0')}:${utcMinutes.toString().padStart(2, '0')}`
    ...
    <View className='publish-modal' onClick={handleMaskClick}>
      <View className='content' onClick={handleContentClick}>
        ...
        {foodData ? (
          ...
          >
            {isSubmitting ? '处理中...' : hasNutrients ? '提交' : '继续'}
          </Button>
        ) : (
