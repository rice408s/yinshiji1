  ...
  const checkLoginStatus = async () => {
    try {
      ...
      if (result.code === 200) {
        ...
        await fetchRecords() // 登���验证成功后加载数据
      } else {
        clearUserInfo()
        redirectTo({ url: '/pages/login/index' })
      }
          ...
          {groupedRecords.map(group => (
            <View key={group.date} className='date-group'>
              <View className='date-label'>
                <Text>{group.date.split('-')[1]}</Text>
                <Text className='month'>{group.date.split('-')[0]}月</Text>
              </View>
                ...
                <View className='records-grid'>
                  {group.records.map(record => (
                    <FoodGridCard
                      ...
                      nutrients={[
                        record.nutrients?.protein && '蛋白质',
                        record.nutrients?.fat && '脂肪',
                        record.nutrients?.carbohydrates && '碳水化合物'
                      ].filter(Boolean)}
