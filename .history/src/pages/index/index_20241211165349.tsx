...
export default function Index() {
  ...
  const handleOpenPublish = () => {
    console.log('打开弹窗，记录当前��置:', lastScrollTopRef.current)
  ...
  const handleModalClose = async (isComplete: boolean) => {
    console.log('弹窗关闭:', {
      ...
      modalOpenScrollTop: modalOpenScrollTopRef.current
    })
    ...
    if (isComplete) {
      ...
      setPage(1)
      handleScrollStart()  // 开始滚动动画
      await fetchRecords(1, true)
      handleScrollEnd()  // 结束滚动动画
    } else {
      console.log('取消操作，恢复到打开时的位置')
      handleScrollStart()  // 开始滚动动画
      nextTick(() => {
  ...
  return (
    <View className='index'>
      <ScrollView
        className={`records-list ${isScrolling ? 'scrolling' : ''}`}
        scrollY
        scrollTop={scrollTop}
        refresherEnabled
